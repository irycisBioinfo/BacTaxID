...
/// * `click_size` – minimum size of the clique to search for.
pub fn exists_clique(
    ...
    for clique in maximal_cliques(&g) {
        if clique.len() >= click_size {
            ...
            /* Retrieve edge IDs of the clique */
...
/// and whose distance is less than `dist_max`.
///
/// # Parameters:
/// - `conn`      : Mutable connection to DuckDB.
/// - `edge_ids`  : Slice of `i64` with the IDs (primary key) to delete.
/// - `dist_min`  : Minimum distance; only edges with `dist < dist_min` are deleted.
///
/// # Returns:
/// - `Ok(())` on success, or `Err` in case of a database error.
pub fn delete_edges_by_ids(
    ...
    // Delete only those edges with id in temp_table and dist < dist_min
    ...
    println!("✓ Deleted {} rows from the edges table", deleted);
    Ok(())
}


/// Gets the edges from `edges` where source or target is in the node_ids slice (both VARCHAR)
