...
/// Verifies that the TOML file exists, reads it, parses it, creates and initializes the database.
/// Returns the DuckDb instance ready to use.
pub fn verify_init_files(toml_path: &str) -> Result<DuckDb> {
    // 1. Verify that the TOML file exists
    Path::new(toml_path)
        .exists()
        .then(|| ())
        .ok_or_else(|| anyhow::anyhow!("The TOML file does not exist: {}", toml_path))?;
    ...
    // 4. Initialize all tables according to the config
    db.init_all_tables_from_config(&config)
        .with_context(|| format!("Error initializing tables in {}", db_name))?;

    // 5. Verify that the database file was created
    Path::new(&db_name)
        .exists()
        .then(|| ())
        .ok_or_else(|| anyhow::anyhow!("Error: the database was not created {}", db_name))?;

    Ok(db)
}

/// Main function of the init command
pub fn init_command(toml_path: &str) -> Result<()> {
    println!("=== Starting init command ===");

    // Uses the verification and initialization function
