/// Structure representing a sketch of a biological sequence
...
impl Sketch {
    ...
    /// Calculates the Mash distance between this sketch and another
...
pub fn load_sketch_manager_from_db(
    ...
    // 2. Collect into a Vec to parallelize
    let raw_data: Result<Vec<_>, _> = rows.collect();
    let raw_data = raw_data?;

    // If there are no rows, create an empty SketchManager
    ...
    // 3. Parallelize deserialization and construction of the HashMap
...
/// - Filters duplicates per sequence.
/// - Iterates in streaming without collecting all hashes.
/// - Derived from BinDash: https://github.com/zhaoxiaofei/bindash
...
pub fn jaccard_index(sketch1: &[u64], sketch2: &[u64], kmer_size: usize) -> f64 {
    ...
    if jaccard_value == 0.0 {
        ...
        // Calculate Mash distance: D = -1/k * ln(2j/(1+j))
        ...
        // Ensure ANI is in the range [0, 1]
        return ani.max(0.0).min(1.0);
    }

}
/// Compares the sketches of query_manager against a specific subset of sketches
/// in reference_manager, returning only those comparisons with distance < max_dist.
///
/// # Arguments
/// - `query_manager`: SketchManager with the source (query) sketches.
/// - `reference_manager`: SketchManager with the reference sketches.
/// - `subset_ids`: List of specific IDs to compare from the reference_manager.
...
/// Compares all sketches in `query_manager` against all sketches in `reference_manager`
/// in parallel, returning only those comparisons with distance < max_dist.
///
/// # Arguments
/// - `query_manager`: Reference to the SketchManager containing the source (query) sketches.
/// - `reference_manager`: Reference to the SketchManager containing the reference (target) sketches.
/// - `max_dist`: Maximum distance. Only distances < max_dist are returned.
///
/// # Returns
/// A vector of tuples (source_name, target_name, distance) where distance < max_dist.
